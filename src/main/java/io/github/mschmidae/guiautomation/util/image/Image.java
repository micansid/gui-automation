 ...
 * Unlike the BuffedImage the ScreenImage overrides the methods equal() and hashCode(). Two ScreenImage representing a equal picture are equal.
 */
@EqualsAndHashCode
...
public class Image {
    ...
    public Image getSubImage(final Section section) {
        return getSubImage(section.getStartPosition().getX(), section.getStartPosition().getY(), section.getWidth(), section.getHeight());
    }
