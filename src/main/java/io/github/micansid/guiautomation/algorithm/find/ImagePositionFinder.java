...
/**
 * Interface of ImagePositionFinders, which provide methods to find the position of images in a
 * image.
 */
public interface ImagePositionFinder {
   ...
   * @return true when the pattern is at the position in the image
   */
  default boolean at(final Image image, final Image pattern, final int positionX,
                     final int positionY) {
    Ensure.notNull(image);
    Ensure.notNull(pattern);
    ...
    for (int y = 0; y < pattern.getHeight() && match; y++) {
      for (int x = 0; x < pattern.getWidth() && match; x++) {
        if (image.getWidth() <= positionX + x || image.getHeight() <= positionY + y
            || (match && image.getRgb(positionX + x, positionY + y) != pattern.getRgb(x, y)
            && !pattern.isTransparent(x, y))) {
          match = false;
        }
      }
    }
