...
class ImageTest {
    private static final BufferedImage IMAGE_COMMIT_BUTTON = new ImageLoader().loadBufferedImage("intellij_button_commit.png").orElseThrow(() -> new RuntimeException("The test image ist not available"));
    private static final BufferedImage IMAGE_CANCEL_BUTTON = new ImageLoader().loadBufferedImage("intellij_button_cancel.png").orElseThrow(() -> new RuntimeException("The test image ist not available"));
    ...
    void returnsSameRGBWidthHeightAsBufferedImage() {
        ...
        for (int y = 0; y < IMAGE_COMMIT_BUTTON.getHeight(); y++) {
            for (int x = 0; x < IMAGE_COMMIT_BUTTON.getWidth(); x++) {
                ...
                assertThat(sut.getRed(new Position(x, y))).isEqualTo(new Color(bufferedImageRgb, true).getRed());
                assertThat(sut.getGreen(x, y)).isEqualTo(new Color(bufferedImageRgb, true).getGreen());
                assertThat(sut.getGreen(new Position(x, y))).isEqualTo(new Color(bufferedImageRgb, true).getGreen());
                assertThat(sut.getBlue(x, y)).isEqualTo(new Color(bufferedImageRgb, true).getBlue());
                assertThat(sut.getBlue(new Position(x, y))).isEqualTo(new Color(bufferedImageRgb, true).getBlue());
                assertThat(sut.getAlpha(x, y)).isEqualTo(new Color(bufferedImageRgb, true).getAlpha());
                assertThat(sut.getAlpha(new Position(x, y))).isEqualTo(new Color(bufferedImageRgb, true).getAlpha());
            }
        }
    }
    ...
    void twoImagesWithEqualBufferedImageAreEqual() {
        BufferedImage commit_button = new ImageLoader().loadBufferedImage("intellij_button_commit.png").orElseThrow(() -> new RuntimeException("The test image ist not available"));
